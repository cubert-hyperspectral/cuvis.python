...
`import cuvis.ipc` works with no CUVIS env var and no cuvis.dll present.
...
_HEAD = struct.Struct("<iiiIQQQQ")   # backend, device, handle_type, blob_len, size, alloc_size, offset, pid
_BLOB_OFF = 48
_BLOB_MAX = 64
...
_MAGIC = b"CVIP"
_VERSION = 1
_PAYLOAD_HDR = struct.Struct("<4sIIIII")  # magic, version, width, height, channels, format_code
...
def pack_payload(descriptor: bytes, width: int, height: int, channels: int, format_code: int) -> bytes:
    ...
    return _PAYLOAD_HDR.pack(_MAGIC, _VERSION, int(width), int(height), int(channels),
                             int(format_code)) + bytes(descriptor)
...
class _CudaArray:
    def __init__(self, ptr, nbytes):
        self.__cuda_array_interface__ = {
            "shape": (nbytes,), "typestr": "|u1", "data": (int(ptr), False), "version": 3,
        }
...
class ImportedCube:
    ...
    def __init__(self, descriptor_bytes: bytes, shape=None, format_code=None):
        (self.backend, self.device, self.htype, blob_len,
         self.size, self.alloc, self.offset, self.pid) = _HEAD.unpack_from(descriptor_bytes, 0)
        ...
        self._ptr_blob = bytes(descriptor_bytes[_PTR_BLOB_OFF:_PTR_BLOB_OFF + ptr_blob_len])
        ...
        if self.backend == BACKEND_POOL:
            ...
            raise NotImplementedError(f"backend {self.backend} is not importable cross-process")
        self._ptr += self.offset  # 0 for pool/legacy/vmm (import returns the exact base pointer)
    ...
    def _open_pool(self):
        ...
        if self.htype == H_WIN32_KMT:
            ...
            htype = runtime.cudaMemAllocationHandleType.cudaMemHandleTypePosixFileDescriptor
        else:
        ...
        (pool,) = _ck(runtime.cudaMemPoolImportFromShareableHandle(handle_val, htype, 0),
        ...
        (ptr,) = _ck(runtime.cudaMemPoolImportPointer(pool, export_data), "cudaMemPoolImportPointer")
    ...
    def _open_legacy(self):
        ...
        (ptr,) = _ck(runtime.cudaIpcOpenMemHandle(h, runtime.cudaIpcMemLazyEnablePeerAccess),
                     "cudaIpcOpenMemHandle")

        def close():
            runtime.cudaIpcCloseMemHandle(ptr)  # release this process's mapping (not the exporter's)
    ...
    def _open_vmm(self):
        ...
        handle_val = int.from_bytes(self._blob, "little")
        (gen,) = _ck(driver.cuMemImportFromShareableHandle(handle_val, htype),
    ...
    def _torch_dtype(self, torch):
        return None if self._format is None else getattr(torch, _TORCH_DTYPE[self._format])
...
__all__ = ["ImportedCube", "open", "open_descriptor", "pack_payload",
           "BACKEND_NONE", "BACKEND_POOL", "BACKEND_LEGACY", "BACKEND_VMM"]
